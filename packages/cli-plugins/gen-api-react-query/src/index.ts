...
} from './utils/parse-type-definitions'
...
export function mergeTypeScriptContent(
  ...
  const removeHeaderComment = (content: string): string => {
    ...
    const removeBlockComment = (str: string): string => {
      const result = str.replace(/^\s*\/\*[\s\S]*?\*\/\s*/, '')
      return result === str ? str : removeBlockComment(result)
    }

    const removeLineComment = (str: string): string => {
  ...
  const newBodyWithoutTypes = (newBody || '').replace(
    new RegExp(TYPE_BLOCK_REGEX.source, TYPE_BLOCK_REGEX.flags),
    '',
  )
  ...
  otherCodeFromNew = otherCodeFromNew.replace(
    /\/\*\*?\s*\*\s*!DO NOT EDIT THIS FILE[\s\S]*?\*\//g,
    '',
  )
  // "tok-cli.config.ts 에서 설정된..." 주석도 제거
  otherCodeFromNew = otherCodeFromNew.replace(
    /\/\*\*?\s*\*\s*tok-cli\.config\.ts[\s\S]*?\*\//g,
    '',
  )
  // 연속된 빈 줄 정리
  otherCodeFromNew = otherCodeFromNew.replace(/\n\s*\n\s*\n+/g, '\n\n').trim()
