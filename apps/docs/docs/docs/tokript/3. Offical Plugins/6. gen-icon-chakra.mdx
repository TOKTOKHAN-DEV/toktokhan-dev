...
<DocIcons 
   github="https://github.com/TOKTOKHAN-DEV/toktokhan-dev/tree/main/packages/cli-plugins/gen-icon-chakra" 
   npm="https://www.npmjs.com/package/@toktokhan-dev/cli-plugin-gen-icon-chakra" 
   doc="/docs/cli-plugin-gen-icon-chakra"
   />



지정된 경로의 svg파일 기반으로 Chakra UI Icon Component를 생성하는 스크립트입니다.<br/>
...
<Installations
  isDev
  package={'@toktokhan-dev/cli @toktokhan-dev/cli-plugin-gen-icon-chakra'}
/>

## Run Script

<TokriptRunScript cmd="gen:icon"/>

## Configuration

<TokriptConfigExample cmd="gen:icon" exportedName="genIcon" packageName="@toktokhan-dev/cli-plugin-gen-icon"/>
...
<TokriptConfigItem
    command="gen:icon"
    optName="input"
    desc="아이콘 파일이 위치한 디렉토리 경로"
    type="string" 
    default="public" 
    exampleValue="path/to/icon"
    cliOpt
    cliOptAlias="i" 
 />
...
<TokriptConfigItem
    ...
    desc="생성될 파일이 위치할 경로"
    type="string" 
    default="src/generated/path/icons.ts" 
    exampleValue="path/to/icon"
    cliOpt 
    cliOptAlias="o" 
 />
...
### ignored
 ...
 <TokriptConfigItem
    command="gen:icon"
    optName="ignored"
    desc="제외 될 아이콘 파일을 판별하는 패턴으로써, 파일이름 또는 폴더명이 패턴과 일치할 경우에 객체에서 제외 됩니다."
    type="string[]" 
    default="['*node_module*']" 
    exampleValue="['/some-folder/*']"
    cliOpt
    cliOptAlias="ig" 
 />
