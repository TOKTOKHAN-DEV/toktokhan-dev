...
const getIndexFilePath = (target: string) => {
  const root = createPackageRoot(path.dirname(target))
  const indexFile = ['index.ts', 'index.tsx', 'src/index.ts', 'src/index.tsx']
    .map(pathOn(root()))
    .find(existsSync)
  if (!indexFile) throw new Error('No index file found')
  return indexFile
}
